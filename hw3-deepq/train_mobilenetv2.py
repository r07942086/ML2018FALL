...
def get_model_spec():
    ...
    """
    return {"model_name": "mobilenetv2", "pretrained": True, "arg_scope_dict": {'weight_decay':0.8}}
...
def get_eval_spec():
    ...
    """
    transform = lambda image: tf.image.resize_images(image, [244, 244,3])
...
def before_epoch(train_history, validation_history):
    ...
    returns:
        a dictionary contains following items:
            ...
            batchsize: an integer between 1 and 64
    """
    transform = lambda image: tf.image.resize_images(image, [224, 224,3])
...
def before_batch(train_history, validation_history):
    ...
    """
    global lr
    return {'feed_dict': {lr: 0.001}, 'batch_norm': 0.9, 'drop_out': 0.25}
